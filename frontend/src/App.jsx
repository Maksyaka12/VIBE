...
function CreatorRevenue() {
  ...
  return (
    <section id="revenue" className="alt">
      <div className="wrap">
        <div className="sec-head rv" ref={r}>
          <h2>Creator <span className="bl">Revenue Economy</span>.</h2>
          <p className="sec-sub">All trading fees generated by the creator on o1 Exchange are reinvested into the $VIBE ecosystem for maximum transparency.</p>
        </div>
        
        <div className="stat-tiles wide-stats">
          <div className="stile"><span className="v">30%</span><span className="l">Buyback & Burn</span><div className="d">Decreasing supply</div></div>
          <div className="stile"><span className="v">70%</span><span className="l">Community Rewards</span><div className="d">Upcoming ecosystem incentives</div></div>
        </div>

        <div className="tok-layout">
          <div style={{width:'100%'}}>
            <div className="tok-card">
              <h3>Revenue Events Transparency</h3>
              <p className="sub" style={{marginBottom:18}}>A dynamic log of all creator revenue actions (buybacks, burns, listing payments).</p>
              
              <div className="rev-events-list">
                {REVENUE_EVENTS.map((ev, i) => (
                  <div key={i} className="rev-event-card">
                    <div className="rev-event-left">
                      <div className={`rev-type-badge ${ev.type}`}>{ev.type.toUpperCase()}</div>
                      <div className="rev-date">{ev.date}</div>
                    </div>
                    <div className="rev-event-mid">
                      <h4>{ev.title}</h4>
                      <div className="rev-amount">{ev.amount}</div>
                    </div>
                    <div className="rev-event-right">
                      {ev.txLink && <a href={ev.txLink} target="_blank" rel="noreferrer" className="btn-line" style={{padding:'6px 12px', fontSize:'0.85rem'}}>Tx <ArrowUpRight size={14}/></a>}
                      {ev.tweetLink && <a href={ev.tweetLink} target="_blank" rel="noreferrer" className="btn-line" style={{padding:'6px 12px', fontSize:'0.85rem'}}>Tweet <ArrowUpRight size={14}/></a>}
                    </div>
                  </div>
                ))}
                {REVENUE_EVENTS.length === 0 && (
                  <p style={{color:'var(--muted)'}}>No events recorded yet. Stay tuned!</p>
                )}
              </div>
            </div>
          </div>
